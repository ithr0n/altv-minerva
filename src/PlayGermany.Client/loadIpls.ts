...
// casino interior
interiorID = natives.getInteriorAtCoords(1100.0, 220.0, -50.0)
if (natives.isValidInterior(interiorID)) {
    natives.activateInteriorEntitySet(interiorID, 'casino_manager_default')
    natives.activateInteriorEntitySet(interiorID, 'casino_manager_workout')
    natives.refreshInterior(interiorID)
}

// casino penthouse interior
interiorID = natives.getInteriorAtCoords(976.636, 70.295, 115.164)
const phPropList = [
    'Set_Pent_Tint_Shell',
    'Set_Pent_Pattern_01',
...
if (natives.isValidInterior(interiorID)) {
    phPropList.forEach((propName) => {
        natives.activateInteriorEntitySet(interiorID, propName)
        natives.setInteriorEntitySetColor(interiorID, propName, 1)
    })
    natives.refreshInterior(interiorID)
}

// fix simeons car store broken glasses
interiorID = natives.getInteriorAtCoordsWithType(-38.62, -1099.01, 27.31, 'v_carshowroom')
if (natives.isValidInterior(interiorID)) {
    natives.activateInteriorEntitySet(interiorID, 'csr_beforeMission')
    natives.activateInteriorEntitySet(interiorID, 'shutter_closed')
    natives.refreshInterior(interiorID)
}